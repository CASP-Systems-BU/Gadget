...
class joinTumbling: public ::testing::Test {
public:
    joinTumbling( ) {
        ...
        {
            // events for event generator 2
            auto e1 = std::make_shared<Event>();
            e1->eventTime_ = 0.1; // time is in second
            e1->key_ = "1";
            generatedEvents2.push_back(e1);

            auto e3 = std::make_shared<Event>();
            e3->eventTime_ = 1.1;
            e3->key_ = "2";
            generatedEvents2.push_back(e3);

            auto e5 = std::make_shared<Event>();
            e5->eventTime_ = 4.2;
            e5->key_ = "3";
            generatedEvents2.push_back(e5);

            auto e6 = std::make_shared<Event>();
            e6->eventTime_ = 5;
            e6->key_ = "4";
            generatedEvents2.push_back(e6);

            auto e8 = std::make_shared<Event>();
            e8->eventTime_ = 6.3;
            e8->key_ = "7";
            generatedEvents2.push_back(e8);
    ...
    void SetUp( ) {
        ...
        eventGeneratorParams->memEvents = generatedEvents2;
        eventGen2 =  EventGeneratorBuilder::BuildEventGenerator(EventGenerator::MemEvents, eventGeneratorParams);
        ...
        operatorParams->operatorType = Operator::OperatorJoinTumbling ;
...
TEST_F(joinTumbling, joinTumblingNumOpr) {
    ...
    genOperations.clear();
    opr->runOperator(genOperations);
    ASSERT_EQ(genOperations.size(), 1);

    // time unit 3
    genOperations.clear();
    opr->runOperator(genOperations);
    EXPECT_EQ(genOperations.size(), 3);
    ...
    genOperations.clear();
    opr->runOperator(genOperations);
    EXPECT_EQ(genOperations.size(), 1);

    // time unit 10
    genOperations.clear();
    opr->runOperator(genOperations);
    EXPECT_EQ(genOperations.size(), 11);
...
// scan ---> 4
// next ---> 5
TEST_F(joinTumbling, joinTumblingOprSequence) {
    std::vector<std::shared_ptr<Operation>> genOperations;
    // time unit 1
    genOperations.clear();
    opr->runOperator(genOperations);
    ...
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); // merge  operation
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); // merge  operation
    ...
    // check the sequence of generated operations
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); //  merge  operation
    EXPECT_EQ( static_cast<int>(genOperations[1]->oprType), 2); //  merge  operation
    EXPECT_EQ( static_cast<int>(genOperations[2]->oprType), 2); //  merge  operation
    ...
    EXPECT_EQ( genOperations[6]->key, "0000000003");
    EXPECT_EQ( genOperations[7]->key, "0000000003");
    ...
    // check the sequence of generated operations
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); //  merge  operation
    EXPECT_EQ( static_cast<int>(genOperations[1]->oprType), 0); //  get  operation
    EXPECT_EQ( static_cast<int>(genOperations[2]->oprType), 3); //  delete  operation
    EXPECT_EQ( static_cast<int>(genOperations[3]->oprType), 0); //  get  operation
    EXPECT_EQ( static_cast<int>(genOperations[4]->oprType), 3); //  delete  operation
    EXPECT_EQ( static_cast<int>(genOperations[5]->oprType), 0); //  get  operation
    EXPECT_EQ( static_cast<int>(genOperations[6]->oprType), 3); //  delete  operation
    EXPECT_EQ( static_cast<int>(genOperations[7]->oprType), 0); //  get  operation
    EXPECT_EQ( static_cast<int>(genOperations[8]->oprType), 3); //  delete  operation
    EXPECT_EQ( static_cast<int>(genOperations[9]->oprType), 0); //  get  operation
    EXPECT_EQ( static_cast<int>(genOperations[10]->oprType), 3); //  delete  operation
    // check the keys generated operations
    EXPECT_EQ( genOperations[0]->key, "0000000008");
    EXPECT_EQ( genOperations[1]->key, "0000000004");
    EXPECT_EQ( genOperations[2]->key, "0000000004");
    EXPECT_EQ( genOperations[3]->key, "0000000005");
    EXPECT_EQ( genOperations[4]->key, "0000000005");
    EXPECT_EQ( genOperations[5]->key, "0000000006");
    EXPECT_EQ( genOperations[6]->key, "0000000006");
    EXPECT_EQ( genOperations[7]->key, "0000000007");
    EXPECT_EQ( genOperations[8]->key, "0000000007");
    EXPECT_EQ( genOperations[9]->key, "0000000008");
    EXPECT_EQ( genOperations[10]->key, "0000000008");
    ...
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); //  merge  operation
    // check the keys generated operations
    EXPECT_EQ( genOperations[0]->key, "0000000009");
...
TEST_F(joinTumbling, latejoinTumblingOprSequence) {
