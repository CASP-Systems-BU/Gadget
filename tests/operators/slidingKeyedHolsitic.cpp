...
class slidingKeyedHolistic: public ::testing::Test {
public:
    slidingKeyedHolistic( ) {
...
TEST_F(slidingKeyedHolistic, slidingKeyedNumOpr) {
    std::vector<std::shared_ptr<Operation>> genOperations;
    // get first batch of state accesses
    // time unit 1
    genOperations.clear();
    opr->runOperator(genOperations);
    // for the first unit of time. There is  only one event; therefore, two operations are generated (a get and a merge)
...
// scan ---> 4
// next ---> 5
TEST_F(slidingKeyedHolistic, slidingKeyedOprSequence) {
    std::vector<std::shared_ptr<Operation>> genOperations;
    genOperations.clear();
    opr->runOperator(genOperations);
    ...
    EXPECT_EQ( static_cast<int>(genOperations[6]->oprType), 0); // delete operation
...
TEST_F(slidingKeyedHolistic, lateslidingKeyedOprSequence) {
    ...
    genOperations.clear();
    opr->runOperator(genOperations);
    EXPECT_EQ(genOperations.size(), 7);
    // check the sequence of generated operations
    EXPECT_EQ( static_cast<int>(genOperations[0]->oprType), 2); // merge operation
    ...
    EXPECT_EQ( static_cast<int>(genOperations[5]->oprType), 0); // delete operation
    ...
    genOperations.clear();
    opr->runOperator(genOperations);
    EXPECT_EQ(genOperations.size(), 9);
    // check the sequence of generated operations
